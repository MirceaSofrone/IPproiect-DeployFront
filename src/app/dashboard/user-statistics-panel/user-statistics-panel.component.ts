...
export class UserStatisticsPanelComponent {
  ...
  private URL = 'https://back-end-hpp.herokuapp.com/api/v1/users/getfavorite/';
  private rawData: any[];
  data: any[] = [{
    name: 'Average Price',
    series: []
  ...
  constructor(private http: HttpClient) {
   const headers = { 'Authorization': 'Bearer ' + localStorage.getItem('token') };
    this.http.get<IUserStatistics[]>(this.URL + localStorage.getItem('userID'), { headers }).subscribe(data => this.rawData = data);
  }
